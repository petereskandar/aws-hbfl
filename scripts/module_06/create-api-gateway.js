  ...
  return new Promise((resolve, reject) => {
    apiG.getResources(params, (err, data) => {
      if(err) reject(err);
        ...
        const rootResource = data.items.find(r => r.path === '/'); // this is the main root
...
function createResourceMethod (resourceId, method, api, path) {
  ...
  return new Promise((resolve, reject) => {
    ...
    apiG.putMethod(params, (resolve, reject) => {
      if(err) reject(err);
...
function createMethodIntegration (resourceId, method, api, path) {
  const params = {
    ...
    type: 'HTTP_PROXY',
    uri: 'http://hamsterELB-1466744753.eu-west-3.elb.amazonaws.com' // uri for the load balancer + protocol
  }
