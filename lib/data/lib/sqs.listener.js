...
function poll () {
  // Create params const for getting queue URL
  const params = {
...
function deleteMsgs (results, queueUrl) {
  const promises = results.map((result) => {
    //  Create params const for deleting message
    const params = {
      QueueUrl: queueUrl,
      ReceiptHandle: result.ReceiptHandle // unique identifier for each recieved message
    }
    return new Promise((resolve, reject) => {
